%% settings_sdm.m
% *Summary:* Script set up the spring damper mass scenario
%
...
S0 = diag([0.01 0.01].^2);   % initial state covariance
...
% 3. Plant structure
plant.dynamics = @dyn_mass_spring_damper;%dynamics_cp;                    % dynamics ode function
...
mm = [mu0]; ss = [S0];         % in complex plane          
policy.p.inputs = gaussian(mm(poli), ss(poli,poli), nc)'; % init. location of 
                                                          % basis functions
policy.p.targets = 0.1*randn(nc, length(policy.maxU));    % init. policy targets 
                                                          % (close to zero)
policy.p.hyp = log([1 1 1 0.001])';              % initialize policy
                                                          % hyper-parameters

% 5. Set up the cost structure
cost.fcn = @loss_sdm;                       % cost function
...
cost.target = [1.5 0]';                 % target state
