%% loss_sdm.m
% *Summary:* spring damper mass loss function; the loss is 
...
function [L, dLdm, dLds, S2] = loss_sdm(cost, m, s)
%% Code
...
M = zeros(D0,1); M(1:D0) = m; S = zeros(D0); S(1:D0,1:D0) = s;
Mdm = [eye(D0)]; Sdm = zeros(D0*D0,D0);
Mds = zeros(D0,D0*D0); Sds = kron(Mdm,Mdm);
