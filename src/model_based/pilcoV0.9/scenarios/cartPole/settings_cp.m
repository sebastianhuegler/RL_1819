...
dt = 0.0250;                         % [s] sampling time
T = 2.5;                           % [s] initial prediction horizon time
H = ceil(T/dt);                    % prediction steps (optimization horizon)
mu0 = [0 0 0 0*pi/180]';                  % initial state mean
S0 = diag([0.001 0.001 0.001 0.001].^2);   % initial state covariance
N = 10;                            % number controller optimizations
J = 1;                             % initial J trajectories of length H
K = 1;                             % no. of initial states for which we optimize
nc = 25;                          % number of controller basis functions

% 3. Plant structure
plant.dynamics = @dyn_cart_pole;%dynamics_cp;                    % dynamics ode function
...
policy.p.targets = 0.1*randn(nc, length(policy.maxU));    % init. policy targets 
                                                          % (close to zero)
policy.p.hyp = log([1 1 1 0.7 0.7 1 0.001])';              % initialize policy
                                                          % hyper-parameters
